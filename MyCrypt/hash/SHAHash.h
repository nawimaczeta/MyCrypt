...
typedef vector<uint8_t> DataBlock;
...
{
public:
	SHAHash(const array<T, 8> & H_initValues, const vector<T> & K_values, uint32_t hashSizeWords, 
...
protected:
	...
	const vector<T> _K;
	const uint32_t _hashSizeWords;
	const uint32_t _numOfAlgorithmRuns;
	const uint32_t _wordSize;
	...
	inline T _WordFromBytes(vector<uint8_t> in) const;
	inline vector<uint8_t> _BytesFromWord(T in) const;

	inline array<uint8_t, 8> _AUINT8fromUINT64(uint64_t in) const;
};

const vector<uint32_t> SHA224_256_K_INIT{
...
const vector<uint64_t> SHA384_512_K_INIT{
...
const array<uint64_t, 8> SHA512_H_INIT{
...
struct HashFactory
{
	static unique_ptr<IHash> Get(enum class HashAlgorythm hashAlgorythm) {
		...
		case HashAlgorythm::SHA224:
			res.reset( new SHAHash<uint32_t>(SHA224_H_INIT, SHA224_256_K_INIT, 7, 64, 64) );
			break;
		case HashAlgorythm::SHA256:
			res.reset( new SHAHash<uint32_t>(SHA256_H_INIT, SHA224_256_K_INIT, 8, 64, 64) );
			break;
		case HashAlgorythm::SHA512:
			res.reset(new SHAHash<uint64_t>(SHA512_H_INIT, SHA384_512_K_INIT, 8, 80, 128));
			break;
		}
